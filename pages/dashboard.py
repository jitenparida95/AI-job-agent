...
def render():
    try:
        ...
        prefs={}; jobs=[]; applied=[]; tracker=[]; settings={}
    ...
    try:
        ...
        user, _ = require_auth()
        name_src = (user.get("user_metadata",{}).get("name") or
                    user.get("name") or user.get("email","").split("@")[0] or "there")
        first = str(name_src).strip().split()[0] if str(name_src).strip() else "there"
    except Exception:
        first = prefs.get("name","there").split()[0] or "there"
...
<div style='padding:8px 0 28px;'>
  <div style='font-family:JetBrains Mono,monospace;font-size:10px;color:#2d4060;letter-spacing:.2em;margin-bottom:4px;'>COMMAND CENTER · {datetime.now().strftime("%d %b %Y  %H:%M")}</div>
  <h1 style='font-size:30px;font-weight:800;margin:0 0 6px;color:#e2e8f0;'>{greeting}, {first} 🚀</h1>
  <p style='color:#4a6080;font-size:14px;margin:0;'>{len(jobs)} jobs in pipeline · {len(all_tracked)} applications tracked.</p>
</div>""", unsafe_allow_html=True)

    # Setup progress
    score = _setup(prefs, settings)
    if score < 100:
        st.markdown(f"""
<div style='background:linear-gradient(135deg,#0f1629,#111827);border:1px solid #1a2540;border-left:3px solid #6366f1;border-radius:12px;padding:18px 22px;margin-bottom:20px;'>
  <div style='font-size:12px;color:#6366f1;font-family:JetBrains Mono,monospace;font-weight:600;margin-bottom:8px;'>SETUP PROGRESS — {score}% COMPLETE</div>
  ...
  <div style='background:#1a2540;border-radius:6px;height:8px;overflow:hidden;'>
    <div style='width:{score}%;height:100%;border-radius:6px;background:linear-gradient(90deg,#6366f1,#22d3a5);'></div>
  </div>
</div>""", unsafe_allow_html=True)

    # Metrics
    scored     = [j for j in jobs if j.get("match_score") is not None]
    high_match = [j for j in scored if (j.get("match_score") or 0) >= settings.get("min_match_score",70)]
    interviews = len([t for t in all_tracked if t.get("status","") in ["Interview","Interview Scheduled","Technical Round"]])
    resp_rate  = f"{int(interviews/max(len(all_tracked),1)*100)}%" if all_tracked else "—"

    c1,c2,c3,c4,c5 = st.columns(5)
    for col, val, label in [
        (c1, len(jobs),        "Jobs in Pipeline"),
        (c2, len(high_match),  "High-Match Roles"),
        (c3, len(all_tracked), "Applications Tracked"),
        (c4, interviews,       "Interviews 🎯"),
        (c5, resp_rate,        "Response Rate"),
    ]:
        with col:
    ...
    for col, (label, ok) in zip(cols, checks.items()):
        ...
        bdr   = "#166534" if ok else "#1a2540"
        col.markdown(f"""<div style='text-align:center;padding:14px 8px;background:{bg};border:1px solid {bdr};border-radius:10px;'>
...
<div style='font-size:13px;color:#7a8faa;'>Run Job Discovery to populate your pipeline with matching FP&A and Finance roles.</div>
</div>""", unsafe_allow_html=True)
    else:
...
<div style='font-size:13px;color:#7a8faa;'>Score your job matches, generate applications, and track your pipeline.</div>
    ...
    # Quick action buttons — actually navigate
    q1,q2,q3,q4 = st.columns(4)
    with q1:
        if st.button("🧠 Career Intelligence", use_container_width=True): _nav("Career Intelligence")
    with q2:
        if st.button("📄 Resume Optimizer", use_container_width=True): _nav("Resume Optimizer")
    with q3:
        if st.button("🔍 Discover Jobs", use_container_width=True): _nav("Job Discovery")
    with q4:
        if st.button("📊 View Tracker", use_container_width=True): _nav("Tracker & Analytics")
    ...
    if all_tracked:
        ...
        for app in list(reversed(all_tracked))[:5]:
            ...
            badge  = status_map.get(status,"badge-pending")
            date_s = (app.get("applied_at") or app.get("created_at",""))[:10]
...
<div style="display:flex;justify-content:space-between;align-items:center;">
  <div>
    ...
    <div class="job-meta">{date_s}</div>
  </div>
  <div style="text-align:right;">
    {f'<div class="match-score {sc}">{score}%</div>' if score else ''}
    <span class="status-badge {badge}">{str(status).upper()}</span>
  </div>
</div>
</div>""", unsafe_allow_html=True)
    else:
...
def _setup(prefs, settings):
    checks = [bool(prefs.get("name")), bool(prefs.get("resume_text") or prefs.get("resume_path")),
              bool(prefs.get("target_roles")), bool(settings.get("groq_api_key")), bool(prefs.get("min_salary_lpa"))]
