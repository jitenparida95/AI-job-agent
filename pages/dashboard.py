...
def render():
    ...
    name = _safe_first_name(user) or (prefs.get("name","").strip().split() or ["there"])[0]
    ...
    for col, val, label in [
        ...
        (c5, response_rate,    "Response Rate"),
    ]:
        with col:
            ...
            st.markdown(f"""<div class="job-card">
                <div style="display:flex; justify-content:space-between; align-items:center;">
                    ...
                    <div style="text-align:right;">
                        ...
                        <span class="status-badge {badge_cls}">{str(status).upper()}</span>
                    </div>
                </div>
            </div>""", unsafe_allow_html=True)
    else:
