...
try:
    ...
    _s = get_settings()
    _changed = False
    # Always prefer secrets over stored value — secrets are authoritative
    if st.secrets.get("GROQ_API_KEY"):
        if _s.get("groq_api_key") != st.secrets["GROQ_API_KEY"]:
            _s["groq_api_key"] = st.secrets["GROQ_API_KEY"]
            _changed = True
    if st.secrets.get("JSEARCH_API_KEY"):
        if _s.get("jsearch_api_key") != st.secrets["JSEARCH_API_KEY"]:
