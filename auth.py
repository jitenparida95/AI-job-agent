...
def _render_auth_page():
    ...
    with col_m:
        st.markdown("""
        <div class='auth-logo'>🚀 CareerOS</div>
        <div class='auth-tagline'>AI CAREER OPERATING SYSTEM</div>
        <div class='price-badge'>✨ Pro Plan — ₹499/month · 7-day free trial</div>
        <div class='feature-list'>
          ✓ Career Intelligence Engine (5 career paths + salary forecast)<br>
          ✓ Resume ATS Optimizer (score, rank, rewrite)<br>
          ✓ Application Engine (cover letter, cold email, LinkedIn DM)<br>
          ✓ AI Career Coach with live chat<br>
          ✓ Full job pipeline tracker & analytics
        </div>
        """, unsafe_allow_html=True)
...
def render_subscription_banner(sub: dict):
    ...
    if sub["status"] == "trial":
        ...
        <div style='background:#111827; border:1px solid {color}; border-radius:8px; padding:12px 14px; margin:12px 0;'>
            ...
            <div style='font-size:11px; color:#5a7090; margin-top:4px;'>₹499/month after trial</div>
        </div>
        """, unsafe_allow_html=True)
        st.sidebar.link_button("⚡ Upgrade to Pro", "https://rzp.io/rzp/StnjPRq", use_container_width=True)
    elif sub["status"] == "expired":
        st.sidebar.error("⚠️ Subscription expired. Please renew.")
        st.sidebar.link_button("🔄 Renew Plan — ₹499/month", "https://rzp.io/rzp/StnjPRq", use_container_width=True)
